...
Zcash payment protocol using googles protocol buffers.
"""
...
from zcash.wallet import CZcashAddress
...
#   the amount for us but a bug in zcash qt version 0.9.1 (at time of
...
#   Also there can be no leading 0's (zeros).
    ...
    headers = {'Content-Type': 'application/zcash-payment',
               'Accept': 'application/zcash-paymentrequest'}
...
def payment_ack(serialized_Payment_message):
    ...
    refund_address = CZcashAddress.from_scriptPubKey(CScript(pao.payment.refund_to[0].script))

    sds_pa = pao.SerializeToString()

    open('sds_pa_blob', 'wb').write(sds_pa)
    headers = {'Content-Type' : 'application/zcash-payment', 'Accept' : 'application/zcash-paymentack'}
