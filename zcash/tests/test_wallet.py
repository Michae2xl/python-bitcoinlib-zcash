...
from zcash.core import b2x, x
from zcash.core.script import CScript, IsLowDERSignature
from zcash.core.key import CPubKey, is_libsec256k1_available, use_libsecp256k1_for_signing
from zcash.wallet import *

class Test_CZcashAddress(unittest.TestCase):
    def test_create_from_string(self):
        """Create CZcashAddress's from strings"""

        def T(str_addr, expected_bytes, expected_version, expected_class):
            addr = CZcashAddress(str_addr)
    ...
    def test_wrong_nVersion(self):
        """Creating a CZcashAddress from a unknown nVersion fails"""

        # tests run in mainnet, so both of the following should fail
        with self.assertRaises(CZcashAddressError):
            CZcashAddress('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')

        with self.assertRaises(CZcashAddressError):
            CZcashAddress('2MyJKxYR2zNZZsZ39SgkCXWCfQtXKhnWSWq')
    ...
    def test_from_scriptPubKey(self):
        def T(hex_scriptpubkey, expected_str_address, expected_class):
            ...
            addr = CZcashAddress.from_scriptPubKey(scriptPubKey)
    ...
    def test_from_nonstd_scriptPubKey(self):
        """CZcashAddress.from_scriptPubKey() with non-standard scriptPubKeys"""
        ...
        with self.assertRaises(CZcashAddressError):
            CZcashAddress.from_scriptPubKey(scriptPubKey)
    ...
    def test_from_invalid_scriptPubKey(self):
        """CZcashAddress.from_scriptPubKey() with invalid scriptPubKeys"""

        # We should raise a CZcashAddressError, not any other type of error
    ...
    def test_to_scriptPubKey(self):
        """CZcashAddress.to_scriptPubKey() works"""
        def T(str_addr, expected_scriptPubKey_hexbytes):
            addr = CZcashAddress(str_addr)
...
class Test_P2PKHBitcoinAddress(unittest.TestCase):
    def test_from_non_canonical_scriptPubKey(self):
        def T(hex_scriptpubkey, expected_str_address):
            ...
            # now test that CZcashAddressError is raised with accept_non_canonical_pushdata=False
            with self.assertRaises(CZcashAddressError):
        ...
        # make sure invalid scripts raise CZcashAddressError
        with self.assertRaises(CZcashAddressError):
    ...
    def test_from_invalid_pubkeys(self):
        ...
        # With accept_invalid=False we should get CZcashAddressError's
        with self.assertRaises(CZcashAddressError):
...
class Test_CZcashSecret(unittest.TestCase):
    def test(self):
        def T(base58_privkey, expected_hex_pubkey, expected_is_compressed_value):
            key = CZcashSecret(base58_privkey)
    ...
    def test_sign(self):
        key = CZcashSecret('5KJvsngHeMpm884wtkJNzQGaCErckhHJBGFsvd3VyK5qMZXj3hS')
        hash = b'\x00' * 32
        sig = key.sign(hash)
...
class Test_RFC6979(unittest.TestCase):
    def test(self):
        ...
        test_vectors = [
            (0x1, "zatoshi Nakamoto", 0x8F8A276C19F4149656B280621E358CCE24F5F52542772691EE69063B74F15D15, "934b1ea10a4b3c1757e2b0c017d0b6143ce3c9a7e6a4a49860d7a6ab210ee3d82442ce9d2b916064108014783e923ec36b49743e2ffa1c4496f01a512aafd9e5"),
            ...
            (0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140, "zatoshi Nakamoto", 0x33A19B60E25FB6F4435AF53A3D42D493644827367E6453928554F43E49AA6F90, "fd567d121db66e382991534ada77a6bd3106f0a1098c231e47993447cd6af2d06b39cd0eb1bc8603e159ef5c20a5c8ad685a45b06ce9bebed3f153d10d93bed5"),
        ...
        for vector in test_vectors:
            secret = CZcashSecret.from_secret_bytes(x('{:064x}'.format(vector[0])))
