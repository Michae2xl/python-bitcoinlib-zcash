"""
`FakeZcashProxy` allows for unit testing of code that normally uses Zcash
RPC without requiring a running Zcash node.

`FakeZcashProxy` has an interface similar to `zcash.rpc.Proxy`, but does not
connect to a local Zcash RPC node. Hence, `FakeZcashProxy` is similar to a
mock for the RPC tests.

`FakeZcashProxy` does _not_ implement a full Zcash RPC node. Instead, it
currently implements only a subset of the available RPC commands. Test setup is
responsible for populating a `FakeZcashProxy` object with reasonable mock
data.
...
from zcash.wallet import (
    # Zcash address initialized from base58-encoded string
    CZcashAddress,
...
def make_address_from_passphrase(passphrase, compressed=True, as_str=True):
    """
    Create a Zcash address from a passphrase. The passphrase is hashed and
    then used as the secret bytes to construct the CZcashSecret.
    """
    if not isinstance(passphrase, bytes):
        passphrase = bytes(passphrase, "utf-8")
    passphrasehash = hashlib.sha256(passphrase).digest()
    private_key = CZcashSecret.from_secret_bytes(passphrasehash, compressed=compressed)
...
def make_txout(amount=None, address=None, counter=None):
    ...
    if not amount:
        maxsatoshis = (21 * 1000 * 1000) * (100 * 1000 * 1000) # 21 million ZEC * 100 million zatoshi per ZEC
        amount = random.randrange(0, maxsatoshis) # between 0 zatoshi and 21 million ZEC

    txout = CMutableTxOut(amount, CZcashAddress(address).to_scriptPubKey())

    return txout

def make_blocks_from_blockhashes(blockhashes):
    """
    Create some block data suitable for FakeZcashProxy to consume during
    instantiation.
    """
    blocks = []
...
class FakeZcashProxyException(Exception):
    """
    Incorrect usage of fake proxy.
    """
    pass

class FakeZcashProxy(object):
    """
    This is an alternative to using `zcash.rpc.Proxy` in tests. This class
    ...
    def getblock(self, blockhash, *args, **kwargs):
        ...
        # Note that the actual "getblock" zcashd RPC call from
    ...
    def fundrawtransaction(self, given_transaction, *args, **kwargs):
        ...
        if isinstance(given_transaction, str):
            ...
            raise FakeZcashProxyException("Wrong type passed to fundrawtransaction.")
    ...
    def signrawtransaction(self, given_transaction):
        ...
        if isinstance(given_transaction, str):
            ...
            raise FakeZcashProxyException("Wrong type passed to signrawtransaction.")
    ...
    def sendrawtransaction(self, given_transaction):
        ...
        if isinstance(given_transaction, str):
            ...
            raise FakeZcashProxyException("Wrong type passed to sendrawtransaction.")
        transaction = CMutableTransaction.deserialize(given_bytes)
    ...
    def _batch(self, batch_request_entries):
        ...
        feature found in python-bitcoinlib and zcashd RPC.
        """
        ...
        for (idx, request) in enumerate(batch_request_entries):
            ...
            for necessary_key in necessary_keys:
                if not necessary_key in request.keys():
                    raise FakeZcashProxyException("Missing necessary key {} for _batch request number {}".format(necessary_key, idx))

            if isinstance(request["params"], list):
                method = getattr(self, request["method"])
                result = method(*request["params"])
            else:
                # matches error message received through python-zcashrpc
