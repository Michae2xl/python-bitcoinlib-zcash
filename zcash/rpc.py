...
from zcash.wallet import CZcashAddress, CZcashSecret
        ...
        if service_url is None:
            ...
            if zcash_conf_file is None:
                if platform.system() == 'Darwin':
                    zcash_conf_file = os.path.expanduser('~/Library/Application Support/Bitcoin/')
                elif platform.system() == 'Windows':
                    zcash_conf_file = os.path.join(os.environ['APPDATA'], 'Bitcoin')
                else:
            ...
            # Treat a missing zcash.conf as though it were empty
            ...
            cookie_dir = conf.get('datadir', os.path.dirname(zcash_conf_file))
            if zcash.params.NAME == 'testnet':
                cookie_dir = os.path.join(cookie_dir, 'testnet3')
            elif zcash.params.NAME == 'regtest':
                cookie_dir = os.path.join(cookie_dir, zcash.params.NAME)
            ...
            try:
                ...
                if 'rpcpassword' in conf:
                    ...
                    raise ValueError('Cookie file unusable (%s) and rpcpassword not specified in the configuration file: %r' % (err, zcash_conf_file))
...
class Proxy(BaseProxy):
    ...
    way. Assumes Zcash version >= v0.16.0; older versions mostly work,
    ...
    def generate(self, numblocks):
        """
        DEPRECATED (will be removed in zcash v0.19)
    ...
    def generatetoaddress(self, numblocks, addr):
        ...
        numblocks - How many blocks are generated immediately.
        addr     - Address to receive block reward (CZcashAddress instance)
    ...
    def getblock(self, block_hash):
        ...
        try:
            ...
            # zcash's rpc takes 0/1/2 instead of true/false as the 2nd argument which specifies verbosity, since v0.15.0.
    ...
    def getrawtransaction(self, txid, verbose=False, block_hash=None):
        ...
        (required when transaction not currently indexed by Zcash)
    ...
    def getreceivedbyaddress(self, addr, minconf=1):
        ...
        addr    - The address. (CZcashAddress instance)
    ...
    def listunspent(self, minconf=0, maxconf=9999999, addrs=None):
        ...
        for unspent in r:
            ...
            # address isn't always available as Zcash allows scripts w/o
            # an address type to be imported into the wallet, e.g. non-p2sh
            # segwit
            try:
                unspent['address'] = CZcashAddress(unspent['address'])
            except KeyError:
                pass
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by zcashd. See
        https://en.zcash.wiki/wiki/BIP_0022 for full specification.
        """
    ...
    def validateaddress(self, address):
        ...
        if r['isvalid']:
            r['address'] = CZcashAddress(r['address'])
        if 'pubkey' in r:
    ...
    def loadwallet(self, name, load_on_startup=False):
        ...
        name - The wallet name.
        load_on_startup - whether to remember to load it automatically next time zcashd starts.
