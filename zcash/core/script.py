...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self):
        return 'CScriptWitness(' + ','.join("x('%s')" % zcash.core.b2x(s) for s in self.stack) + ')'
...
def FindAndDelete(script, sig):
    """Consensus critical, see FindAndDelete() in zatoshi codebase"""
    r = b''
...
def RawSignatureHash(script, txTo, inIdx, hashtype):
    ...
    txtmp.wit = zcash.core.CTxWitness()
    s = txtmp.serialize()
...
def SignatureHash(script, txTo, inIdx, hashtype, amount=None, sigversion=SIGVERSION_BASE):
    ...
    if sigversion == SIGVERSION_WITNESS_V0:
        ...
        if not (hashtype & SIGHASH_ANYONECANPAY):
            ...
            hashPrevouts = zcash.core.Hash(serialize_prevouts)

        if (not (hashtype & SIGHASH_ANYONECANPAY) and (hashtype & 0x1f) != SIGHASH_SINGLE and (hashtype & 0x1f) != SIGHASH_NONE):
            serialize_sequence = bytes()
            for i in txTo.vin:
                serialize_sequence += struct.pack("<I", i.nSequence)
            hashSequence = zcash.core.Hash(serialize_sequence)

        if ((hashtype & 0x1f) != SIGHASH_SINGLE and (hashtype & 0x1f) != SIGHASH_NONE):
            serialize_outputs = bytes()
            for o in txTo.vout:
                serialize_outputs += o.serialize()
            hashOutputs = zcash.core.Hash(serialize_outputs)
        ...
        return zcash.core.Hash(f.getvalue())
