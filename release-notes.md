...
Zcash `-rpcserialversion=0` option, either as a command line argument,
or in your `zcash.conf` file. Otherwise the RPC interface will return
...
* Where appropriate, `RPC_INVALID_ADDRESS_OR_KEY` errors are now caught
  ...
  all!) of the types of RPC errors Zcash returns.

Bugfixes:

* Fixed a spurious `AttributeError` when `zcash.rpc.Proxy()` fails.
...
* RPC over SSL support removed to match Zcash's removal of RPC SSL
...
* SCRIPT_VERIFY_EVEN_S renamed to SCRIPT_VERIFY_LOW_S to match Zcash's naming

* SCRIPT_VERIFY_NOCACHE removed as Zcash no longer has it (and we never
...
* Signing and verification of Zcash compatible messages. (w/ pubkey recovery)
* Tox tests
* Sphinx docs
...
* No longer assumes zcash.conf specifes rpcuser
...
* Improve Zcash address handling. P2SH and P2PKH addresses now get their own
